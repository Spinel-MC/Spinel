...
pub(crate) fn build() -> TokenStream {
    println!(
        "cargo:rerun-if-changed=build_assets/datapacks/default/data/minecraft/wolf_sound_variant/"
    );

    let wolf_sound_variant_dir = "build_assets/datapacks/default/data/minecraft/wolf_sound_variant";
    ...
    for entry in fs::read_dir(wolf_sound_variant_dir).unwrap() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            if wolf_sound_variant_name.starts_with('_') {
                continue;
            }
