...
pub(crate) fn build() -> TokenStream {
    println!("cargo:rerun-if-changed=build_assets/datapacks/default/data/minecraft/frog_variant/");

    let frog_variant_dir = "build_assets/datapacks/default/data/minecraft/frog_variant";
    ...
    for entry in fs::read_dir(frog_variant_dir).unwrap() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            if frog_variant_name.starts_with('_') {
                continue;
            }
