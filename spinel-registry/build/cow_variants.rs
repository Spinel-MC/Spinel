...
pub(crate) fn build() -> TokenStream {
    println!("cargo:rerun-if-changed=build_assets/datapacks/default/data/minecraft/cow_variant/");

    let cow_variant_dir = "build_assets/datapacks/default/data/minecraft/cow_variant";
    ...
    for entry in fs::read_dir(cow_variant_dir).unwrap() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            if cow_variant_name.starts_with('_') {
                continue;
            }
