...
pub(crate) fn build() -> TokenStream {
    println!("cargo:rerun-if-changed=build_assets/datapacks/default/data/minecraft/pig_variant/");

    let pig_variant_dir = "build_assets/datapacks/default/data/minecraft/pig_variant";
    ...
    for entry in fs::read_dir(pig_variant_dir).unwrap() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            if pig_variant_name.starts_with('_') {
                continue;
            }
