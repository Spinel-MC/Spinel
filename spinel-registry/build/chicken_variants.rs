...
pub(crate) fn build() -> TokenStream {
    println!(
        "cargo:rerun-if-changed=build_assets/datapacks/default/data/minecraft/chicken_variant/"
    );

    let chicken_variant_dir = "build_assets/datapacks/default/data/minecraft/chicken_variant";
    ...
    for entry in fs::read_dir(chicken_variant_dir).unwrap() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            if chicken_variant_name.starts_with('_') {
                continue;
            }
