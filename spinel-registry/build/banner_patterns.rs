...
pub(crate) fn build() -> TokenStream {
    println!(
        "cargo:rerun-if-changed=build_assets/datapacks/default/data/minecraft/banner_pattern/"
    );

    let banner_pattern_dir = "build_assets/datapacks/default/data/minecraft/banner_pattern";
    ...
    for entry in fs::read_dir(banner_pattern_dir).unwrap() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            if banner_pattern_name.starts_with('_') {
                continue;
            }
