...
fn entity_type_registry_entry_exposes_the_extracted_minestom_data() {
    ...
    assert_eq!(registry.fire_immune(), entity_type.fire_immune());
    assert_eq!(registry.drag(), entity_type.drag());
    assert_eq!(registry.acceleration(), entity_type.acceleration());
    assert_eq!(
        registry.horizontal_air_resistance(),
        entity_type.horizontal_air_resistance()
    );
    assert_eq!(
        registry.vertical_air_resistance(),
...
fn synchronization_only_entity_types_match_minestom() {
    ...
    assert!(
        synchronization_only_entity_types
            .iter()
            .all(|entity_type| entity_type.synchronizes_position_only())
    );
    assert!(
        EntityType::ALL
            .iter()
            .filter(|entity_type| entity_type.synchronizes_position_only())
            .all(|entity_type| synchronization_only_entity_types.contains(entity_type))
    );
    assert!(!EntityType::ZOMBIE.synchronizes_position_only());
    assert!(!EntityType::PLAYER.synchronizes_position_only());
}
