...
impl Registries {
    ...
    ) -> Vec<(Identifier, Vec<(Identifier, Option<spinel_nbt::NbtCompound>)>)> {
        vec![
            (BIOME_REGISTRY, self.biomes.registry_packet_entries(exclude_vanilla)),
            (CHAT_TYPE_REGISTRY, self.chat_types.registry_packet_entries(exclude_vanilla)),
            (TRIM_PATTERN_REGISTRY, self.trim_patterns.registry_packet_entries(exclude_vanilla)),
            (TRIM_MATERIAL_REGISTRY, self.trim_materials.registry_packet_entries(exclude_vanilla)),
            (WOLF_VARIANT_REGISTRY, self.wolf_variants.registry_packet_entries(exclude_vanilla)),
            (WOLF_SOUND_VARIANT_REGISTRY, self.wolf_sound_variants.registry_packet_entries(exclude_vanilla)),
            (PIG_VARIANT_REGISTRY, self.pig_variants.registry_packet_entries(exclude_vanilla)),
            (FROG_VARIANT_REGISTRY, self.frog_variants.registry_packet_entries(exclude_vanilla)),
            (CAT_VARIANT_REGISTRY, self.cat_variants.registry_packet_entries(exclude_vanilla)),
            (COW_VARIANT_REGISTRY, self.cow_variants.registry_packet_entries(exclude_vanilla)),
            (CHICKEN_VARIANT_REGISTRY, self.chicken_variants.registry_packet_entries(exclude_vanilla)),
            (PAINTING_VARIANT_REGISTRY, self.painting_variants.registry_packet_entries(exclude_vanilla)),
            (DIMENSION_TYPE_REGISTRY, self.dimension_types.registry_packet_entries(exclude_vanilla)),
            (DAMAGE_TYPE_REGISTRY, self.damage_types.registry_packet_entries(exclude_vanilla)),
            (BANNER_PATTERN_REGISTRY, self.banner_patterns.registry_packet_entries(exclude_vanilla)),
            (JUKEBOX_SONG_REGISTRY, self.jukebox_songs.registry_packet_entries(exclude_vanilla)),
            (INSTRUMENT_REGISTRY, self.instruments.registry_packet_entries(exclude_vanilla)),
            (DIALOG_REGISTRY, self.dialogs.registry_packet_entries(exclude_vanilla)),
            (ZOMBIE_NAUTILUS_VARIANT_REGISTRY, self.zombie_nautilus_variants.registry_packet_entries(exclude_vanilla)),
            (TIMELINE_REGISTRY, self.timelines.registry_packet_entries(exclude_vanilla)),
            (ENCHANTMENT_REGISTRY, self.enchantments.registry_packet_entries(exclude_vanilla)),
        ]
    }
    ...
    pub fn register_dynamic_vanilla(&mut self) {
        ...
        vanilla_zombie_nautilus_variants::register_zombie_nautilus_variants(&mut self.zombie_nautilus_variants);
    }
}
