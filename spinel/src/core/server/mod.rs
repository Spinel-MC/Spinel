...
use spinel_network::{ConnectionManager, ServerContext, Player, Client, PacketListener, ConnectionState};
...
impl MinecraftServer {
    pub fn new() -> Self {
        let mut listeners: HashMap<(ConnectionState, i32), Vec<&'static PacketListener>> = HashMap::new();
        let mut generic_listeners: HashMap<ConnectionState, Vec<&'static PacketListener>> = HashMap::new();
        ...
        let explicitly_imported_raw: HashSet<&'static str> = inventory::iter::<&'static RegisteredModule>()
            .map(|m| m.name)
            .collect();
        ...
        let registered_independent_events: HashSet<&'static str> = inventory::iter::<&'static RegisteredEvent>()
            .filter(|e| e.is_independent)
            .map(|e| e.name)
            .collect();

        for listener in inventory::iter::<&'static PacketListener>() {
            let module_ok = listener.modules.is_empty() || listener.modules.iter().any(|unqualified_name| {
                let resolved_name = resolve_ambiguous_module(unqualified_name, &all_known_modules);
                resolved_modules.contains(&resolved_name)
            });

            let event_ok = listener.events.is_empty() || listener.events.iter().any(|event_name| {
                registered_independent_events.contains(event_name)
            });

            if module_ok && event_ok {
                if listener.id == -1 {
                    generic_listeners.entry(listener.state).or_default().push(listener);
                } else {
    ...
    pub async fn start(self, address: &str, port: u16) {
        ...
        let server_handle = tokio::spawn(async move {
            start_tcp_listener(server_arc, &address_owned, port).await
        });
    ...
    pub fn disconnect(&mut self, client: &mut Client, reason: impl Into<TextComponent>) {
        ...
        match client.state {
            ...
            ConnectionState::Configuration => ConfigurationDisconnectPacket::new(reason).dispatch(client),
    ...
    pub fn get_player_for_client_mut<'a>(&'a mut self, client: &'a Client) -> Option<&'a mut Player> {
...
impl ServerContext for MinecraftServer {
    ...
    fn dispatch_packet(&mut self, packet_id: i32, client: &mut Client, payload: Vec<u8>) -> bool {
        ...
        let generic_listeners = self.generic_listeners.get(&client.state).cloned().unwrap_or_default();
