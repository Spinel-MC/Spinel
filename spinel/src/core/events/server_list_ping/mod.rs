...
use crate::core::{events::server_list_ping::{player_info::PlayerSample, server_list_ping_type::ServerListPingType}};
...
impl ServerListPingEventResponseData {
        ...
        pub fn to_status_response_json(&self, hide_players: bool) -> String {
                ...
                insert_if_some(&mut version_json_map, "name", self.brand.as_ref().map(|s| Value::from(s.as_str())));
                ...
                if !hide_players {
                        ...
                        insert_if_some(&mut players_json_map, "max", self.max_players.map(Value::from));
                        insert_if_some(&mut players_json_map, "online", self.online_players.map(Value::from));
                ...
                let description_json_value = self.description
                        .as_ref()
                        .map(|description_component_ref| {
                            serde_json::to_value(description_component_ref)
                                .unwrap_or_else(|e| {
                ...
                insert_if_some(&mut root_json_map, "favicon", self.favicon.as_ref().map(|s| Value::from(s.as_str())));

                insert_if_some(&mut root_json_map, "enforcesSecureChat", self.enforce_secure_chat.map(Value::from));
