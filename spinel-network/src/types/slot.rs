...
impl Slot {
    ...
    pub fn to_item_stack(&self) -> ItemStack {
        ...
        self.components
            .to_supported_data_component_map()
            .ok()
            .map(|components| {
                ItemStack::from_component_patch(material.clone(), self.count, components)
            })
            .unwrap_or_else(|| ItemStack::of(material).with_amount(self.count))
    }
}
