use spinel::{core::{events::{login::PreLoginEvent, server_list_ping::{player_info::PlayerSample, ServerListPingEvent, ServerListPingEventResponseData}}, server::MinecraftServer}, spinel_macros::{event_dispatcher, event_listener, import_module}, utils::{component::{color::{NamedTextColor, TextColor}, Component}, Priority}};
use uuid::Uuid;
...
fn on_pre_login(event: &mut PreLoginEvent, _server: &mut MinecraftServer) {
    ...
    println!("PreLoginEvent: User {} is attempting to log in.", event.name);
}
...
fn on_event(event: &mut ServerListPingEvent, _server: &mut MinecraftServer) {
    ...
    let sample = vec![PlayerSample::new(Component::text("lol".to_string()).color(TextColor::from_named(NamedTextColor::Aqua)).into(), Uuid::new_v4())];

    event.response_data = ServerListPingEventResponseData {
        online_players: Some(0),
        max_players: Some(-1),
        description: Some(Component::text("Minecraft, your way!".to_owned()).color(TextColor{ value: "#ff47d7".to_owned() }).into()),
