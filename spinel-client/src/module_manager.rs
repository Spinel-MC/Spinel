...
use spinel_events::{RegisteredClientModule, RegisteredEvent, RegisteredModuleDependency}; // Fix import

pub struct ClientModuleManager {
    pub independent_events: HashSet<&'static str>,
    pub active_unqualified_modules: HashSet<&'static str>,
}

impl ClientModuleManager {
    pub fn new() -> Self {
        // Gather all unique module names.
        let mut all_module_names: HashSet<&'static str> = HashSet::new();
        for module in inventory::iter::<&'static RegisteredClientModule>() {
        ...
        let mut modules_to_resolve: HashSet<&'static str> =
            inventory::iter::<&'static RegisteredClientModule>()
        ...
        let mut active_unqualified_modules: HashSet<&'static str> = HashSet::new();

        active_unqualified_modules.extend(
            inventory::iter::<&'static ClientPacketListener>()
