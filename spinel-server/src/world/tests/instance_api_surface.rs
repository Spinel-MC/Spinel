...
use crate::world::{
    Block, BlockHandler, BlockHandlerDestroy, BlockHandlerPlacement, BlockHandlerTick,
    BlockInstance, BlockLookupCondition, BlockPlacementRule, BlockPlacementState, BlockPosition,
    BlockReplacement, BlockUpdateState, BossBar, Chunk, ChunkLoader, ChunkPosition, Explosion,
    Weather, World, WorldBorder, WorldBossBarColor, WorldBossBarOverlay, WorldManager,
    WorldSoundEmitter,
};
...
fn weather_validates_levels_and_matches_minestom_immutable_updates() {
    ...
    assert_eq!(
        Weather::CLEAR.with_rain_level(0.5).unwrap(),
        Weather::new(0.5, 0.0).unwrap()
    );
    assert_eq!(
        Weather::RAIN.with_thunder_level(0.75).unwrap(),
        Weather::new(1.0, 0.75).unwrap()
    );
    assert_eq!(
        Weather::THUNDER
            .map_rain_level(|rain_level| rain_level * 0.5)
            .unwrap(),
        Weather::new(0.5, 1.0).unwrap()
    );
    assert_eq!(
        Weather::THUNDER
            .map_thunder_level(|thunder_level| thunder_level * 0.5)
            .unwrap(),
        Weather::new(0.5, 1.0).unwrap()
    );
}
...
fn world_weather_transition_ticks_follow_minestom_overload() {
    ...
    assert!(
        world
            .set_weather_with_transition(Weather::new(1.0, 0.0).unwrap(), 0)
            .is_err()
    );
    world
        .set_weather_with_transition(Weather::new(1.0, 0.5).unwrap(), 4)
        .unwrap();
    world.tick();
...
fn block_handler_placement_preserves_non_default_block_state() {
    ...
    assert!(world.place_block_with_updates(
        BlockHandlerPlacement::new(block_state, Block::AIR, world.uuid(), block_position),
        true,
    ));
    assert_eq!(
        world.loaded_block_state_at(block_position),
        Some(block_state)
    );
}
...
fn block_replacement_uses_generated_flags_and_registered_self_replacement_rules() {
    ...
    let replacement = BlockReplacement::new(
        Block::STONE,
        crate::events::player_block_interact::BlockFace::Top,
        (0.25, 0.5, 0.75),
        false,
        spinel_registry::Material::STONE,
    );

    assert!(Block::SHORT_GRASS.is_replaceable());
    assert!(!Block::STONE.is_replaceable());
    assert!(world.block_is_self_replaceable(replacement));
}
...
fn block_entity_data_follows_the_written_block_value_and_is_removed_on_break() {
    ...
    world
        .set_block_instance(
            block_position,
            BlockInstance::from(Block::CHEST).with_nbt(Some(nbt.clone())),
        )
        .unwrap();
    ...
    assert_eq!(
        world
            ...
            .map(|block_entity| (
                ...
                block_entity.nbt().is_empty()
            )),
        Some((BlockEntityType::TrappedChest, true))
    );
...
fn block_entity_packet_uses_registry_type_and_handler_client_tag_filter() {
    ...
    world
        .set_block_instance(
            block_position,
            BlockInstance::from(Block::CHEST).with_nbt(Some(block_entity_nbt)),
        )
        .unwrap();
    ...
    let client_block_entity_nbt = world.client_block_entity_nbt_at(block_position).unwrap();

    assert_eq!(
        packet_ids,
        vec![BlockUpdatePacket::get_id(), BlockEntityDataPacket::get_id()]
    );
    assert_eq!(decoded_packet.block_entity_type.0, BlockEntityType::Chest);
    assert!(
        decoded_packet
            .data
            .0
    ...
    assert!(client_block_entity_nbt.contains_key("allowed"));
    assert!(!client_block_entity_nbt.contains_key("server_only"));
}
...
fn place_block_sends_block_update_to_all_chunk_viewers() {
    ...
    let mut second_client = test_client();
    first_client.state = ConnectionState::Play;
    second_client.state = ConnectionState::Play;
    first_client.enable_outbound_packet_queue();
    second_client.enable_outbound_packet_queue();
    let mut first_player = Player::new(
        Uuid::new_v4(),
        "FirstViewer".to_string(),
        0,
        SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 25565),
    );
    let mut second_player = Player::new(
        Uuid::new_v4(),
        "SecondViewer".to_string(),
        1,
        SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 25566),
    );
    ...
    second_player.mark_entered_world();
    world.load_chunk(chunk_position).unwrap();
    world
        .load_chunk(chunk_position)
        .unwrap()
        .add_viewer(first_player_id);
    world
        .load_chunk(chunk_position)
        .unwrap()
        .add_viewer(second_player_id);
    world.add_entity(Entity::Player(first_player));
    world.add_entity(Entity::Player(second_player));
    first_client.discard_queued_outbound_packets();
    second_client.discard_queued_outbound_packets();
    ...
    assert_eq!(
        first_client.queued_outbound_packet_ids(),
        vec![BlockUpdatePacket::get_id()]
    );
    assert_eq!(
        second_client.queued_outbound_packet_ids(),
        vec![BlockUpdatePacket::get_id()]
    );
}
...
fn unchanged_world_snapshots_share_chunk_section_storage_until_mutation() {
    ...
    let first_chunk_snapshot = first_snapshot.chunk(chunk_position).unwrap();
    let second_chunk_snapshot = second_snapshot.chunk(chunk_position).unwrap();
    let live_chunk_section_storage = world
        .chunk(chunk_position)
        .unwrap()
        .section_storage_address();

    assert_eq!(
        first_chunk_snapshot.section_storage_address(),
        second_chunk_snapshot.section_storage_address()
    );
    assert_eq!(
        first_chunk_snapshot.section_storage_address(),
        live_chunk_section_storage
    );

    world.set_block(block_position, Block::STONE).unwrap();

    let changed_snapshot = world.update_snapshot();
    let changed_chunk_snapshot = changed_snapshot.chunk(chunk_position).unwrap();

    assert_ne!(
        first_chunk_snapshot.section_storage_address(),
        changed_chunk_snapshot.section_storage_address()
    );
    assert_eq!(first_chunk_snapshot.block(block_position), Block::AIR);
    assert_eq!(changed_chunk_snapshot.block(block_position), Block::STONE);
}
...
impl BlockPlacementRule for SelfReplaceableRule {
    fn block(&self) -> Block {
        self.source_block
    }
    ...
    fn is_self_replaceable(&self, replacement: BlockReplacement) -> bool {
        replacement.block() == self.source_block
            && replacement.block_face() == crate::events::player_block_interact::BlockFace::Top
            && replacement.cursor_position() == (0.25, 0.5, 0.75)
            && !replacement.is_offset()
            && replacement.material() == &spinel_registry::Material::STONE
    }
}
