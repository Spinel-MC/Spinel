...
fn target_is_within_interaction_range(player: &crate::entity::Player, target: &Entity) -> bool {
    ...
    let half_width = target_bounding_box.width() / 2.0;
    let minimum_x = target_position.x() - half_width;
    let maximum_x = target_position.x() + half_width;
    let minimum_y = target_position.y();
    let maximum_y = target_position.y() + target_bounding_box.height();
    let minimum_z = target_position.z() - half_width;
    let maximum_z = target_position.z() + half_width;
