...
fn on_encryption_response(
    ...
    let owned_data = {
        ...
        let private_key = match metadata.private_key.clone() {
            Some(pk) => pk,
            None => {
                return disconnect_with_error(
                    server,
                    client,
                    &format!(
                        "Error: Private key missing in metadata for {}.",
                        client.addr
                    ),
                );
            }
        ...
        let expected_verify_token = match metadata.verify_token.clone() {
            ...
            None => {
                return disconnect_with_error(
                    ...
                    client,
                    &format!(
                        "Error: Verify token missing in metadata for {}.",
                        client.addr
                    ),
                );
            }
        ...
        let username = match metadata.username.clone() {
            ...
            None => {
                return disconnect_with_error(
                    server,
                    client,
                    &format!("Error: Username missing in metadata for {}.", client.addr),
                );
            }
        };
        ...
        let uuid = match metadata.uuid {
            ...
            None => {
                return disconnect_with_error(
                    server,
                    client,
                    &format!("Error: UUID missing in metadata for {}.", client.addr),
                );
            }
        };
    ...
    // Convert Array<u8> (or Array<i8>) to Vec<u8> for rsa
    let key_bytes: Vec<u8> = packet.keybytes.iter().map(|&b| b as u8).collect();
    let challenge_bytes: Vec<u8> = packet
        .encrypted_challenge
        .iter()
        .map(|&b| b as u8)
        .collect();

    let decrypted_shared_secret = match private_key.decrypt(Pkcs1v15Encrypt, &key_bytes) {
        Ok(s) => s,
        Err(e) => {
            return disconnect_with_error(
                server,
                client,
                &format!("Failed to decrypt shared secret for {}: {}", client.addr, e),
            );
        }
    };

    let decrypted_verify_token = match private_key.decrypt(Pkcs1v15Encrypt, &challenge_bytes) {
        Ok(t) => t,
        Err(e) => {
            return disconnect_with_error(
                server,
                client,
                &format!("Failed to decrypt verify token for {}: {}", client.addr, e),
            );
        }
    };
    ...
    server.connection_manager.register_player(
        client.addr,
        Player::new(uuid, username.clone(), protocol_version, client.addr),
    );
