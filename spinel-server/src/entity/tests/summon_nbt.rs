...
fn summon_nbt_applies_position_rotation_name_and_base_flags() {
    ...
    assert_eq!(entity.position().x(), 1.5);
    assert_eq!(entity.position().y(), 70.0);
    assert_eq!(entity.position().z(), -2.5);
    assert_eq!(entity.position().yaw(), 90.0);
    assert_eq!(entity.position().pitch(), 15.0);
    assert_eq!(entity.custom_name().unwrap().to_plain_string(), "Guide");
...
fn summon_nbt_applies_living_and_type_specific_state_only_to_matching_entities() {
    ...
    assert_eq!(armor_stand.health(), 7.5);
    ...
    assert_eq!(slime.health(), 12.0);

    let mut item = GenericEntity::new(EntityType::ITEM);
    item.apply_summon_nbt(&slime_nbt);
    assert_eq!(item.health(), 20.0);
    assert_eq!(item.slime_size(), 1);
}
