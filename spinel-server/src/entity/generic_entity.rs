...
impl GenericEntity {
    ...
    pub const fn get_identity(&self) -> EntityIdentity {
        EntityIdentity::new(self.state.get_uuid())
    }

    pub const fn get_pointers(&self) -> EntityPointers {
        EntityPointers::new(self.state.get_uuid(), self.state.get_entity_id())
    }
    ...
    pub fn as_hover_event(&self) -> HoverEvent {
        HoverEvent::ShowEntity(HoverEntity {
            id: self.state.get_entity_type().key().to_string(),
            uuid: self.state.get_uuid(),
            name: self.get_custom_name().map(Box::new),
        })
    }

    pub fn get_event_node(&mut self) -> &mut EntityEventNode {
        &mut self.event_node
    }
    ...
    pub fn update_snapshot(&self, updater: impl FnOnce(&mut EntitySnapshot)) -> EntitySnapshot {
        let mut snapshot = EntitySnapshot::new(
            ...
            self.state.get_world(),
            self.state.is_removed(),
            self.state.get_metadata().clone(),
            self.get_custom_name(),
            self.state
                .get_world()
                .map(|_| ChunkPosition::from(self.state.get_position())),
            self.state.get_view().get_viewers(),
            self.state.get_passengers().clone(),
            self.state.get_vehicle(),
            self.tag_handler.clone(),
        );
    ...
    pub fn get_metadata_mut(&mut self) -> &mut MetadataHolder {
        self.state.get_metadata_mut()
    }

    pub const fn get_entity_meta(&self) -> EntityMetaRef<'_> {
        EntityMetaRef::new(&self.state)
    }

    pub fn get_entity_meta_mut(&mut self) -> EntityMetaCast<'_> {
        EntityMetaCast::new(&mut self.state)
    }
        ...
        T: DataComponentValue,
    {
        ...
        if component.id() == spinel_registry::data_components::vanilla_components::CAT_COLLAR.id()
            && self.state.get_entity_type() == EntityType::CAT
        {
        ...
        if component.id() == spinel_registry::data_components::vanilla_components::WOLF_COLLAR.id()
            && self.state.get_entity_type() == EntityType::WOLF
        {
        ...
        if component.id()
            ...
            && self.state.get_entity_type() == EntityType::SHULKER
        {
        ...
        if component.id() == spinel_registry::data_components::vanilla_components::FOX_VARIANT.id()
            && self.state.get_entity_type() == EntityType::FOX
        {
        ...
        if component.id() == spinel_registry::data_components::vanilla_components::SALMON_SIZE.id()
            && self.state.get_entity_type() == EntityType::SALMON
        {
        ...
        if component.id()
            ...
            && self.state.get_entity_type() == EntityType::PARROT
        {
        ...
        if component.id()
            ...
            && self.state.get_entity_type() == EntityType::TROPICAL_FISH
        {
        ...
        if component.id()
            ...
            && self.state.get_entity_type() == EntityType::MOOSHROOM
        {
        ...
        if component.id()
            ...
            && self.state.get_entity_type() == EntityType::RABBIT
        {
        ...
        if component.id()
            ...
            && self.state.get_entity_type() == EntityType::HORSE
        {
        ...
        if component.id()
            ...
            && (self.state.get_entity_type() == EntityType::LLAMA
                || self.state.get_entity_type() == EntityType::TRADER_LLAMA)
        {
        ...
        if component.id()
            ...
            && self.state.get_entity_type() == EntityType::AXOLOTL
        {
    ...
    pub const fn get_data_components(&self) -> &DataComponentMap {
    ...
    pub fn is_viewer(&self, viewer_id: EntityId) -> bool {
        self.state.get_view().is_viewer(viewer_id)
    }
    ...
    pub const fn get_previous_position(&self) -> EntityPosition {
        self.state.get_previous_position()
    }

    pub fn set_position(&mut self, position: EntityPosition) {
        self.state.set_position(position);
        if self.state.get_world().is_none() {
            self.synchronization
                .record_position(self.state.get_position());
        }
    }
    ...
    pub fn teleport_with_request(&mut self, request: EntityTeleportRequest) -> EntityTeleport {
    ...
    pub const fn get_head_rotation(&self) -> f32 {
        self.state.get_position().get_head_yaw()
    }

    pub fn set_view(&mut self, yaw: f32, pitch: f32, head_yaw: f32) {
        self.set_position(
            EntityPosition::new(
                self.state.get_position().get_x(),
                self.state.get_position().get_y(),
                self.state.get_position().get_z(),
                yaw,
                pitch,
            )
    ...
    pub fn look_at_position(&mut self, target: EntityPosition) {
        let delta_x = target.get_x() - self.state.get_position().get_x();
        let delta_y = target.get_y() - self.state.get_position().get_y();
        let delta_z = target.get_z() - self.state.get_position().get_z();
    ...
    pub fn look_at_entity(&mut self, entity: &Self) {
        self.look_at_position(entity.get_position());
    }

    pub fn get_chunk(&self) -> ChunkPosition {
        ChunkPosition::new(
            (self.state.get_position().get_x().floor() as i32).div_euclid(16),
            (self.state.get_position().get_z().floor() as i32).div_euclid(16),
        )
    }
    ...
    pub fn get_distance_squared_to_position(&self, position: EntityPosition) -> f64 {
        self.state.get_position().get_distance_squared(position)
    }
    ...
    pub fn has_velocity(&self) -> bool {
        let velocity = self.state.get_velocity().0;
        if self.on_ground {
    ...
    pub(crate) fn set_vehicle(&mut self, vehicle_id: EntityId) {
        if vehicle_id == self.state.get_entity_id() {
            return;
        }
        self.state.set_vehicle(vehicle_id);
    }
    ...
    pub(crate) fn add_passenger(&mut self, passenger_id: EntityId) -> bool {
        if passenger_id == self.state.get_entity_id() {
            return false;
        }
        self.state.add_passenger(passenger_id)
    }

    pub(crate) fn remove_passenger(&mut self, passenger_id: EntityId) -> bool {
        self.state.remove_passenger(passenger_id)
    }

    pub fn has_passenger(&self) -> bool {
        !self.state.get_passengers().is_empty()
    }

    pub fn get_passengers(&self) -> &BTreeSet<EntityId> {
        self.state.get_passengers()
    }

    pub(crate) fn get_passenger_packet(&self) -> SetPassengersPacket {
        SetPassengersPacket {
            vehicle_entity_id: self.state.get_entity_id().get_value(),
    ...
    pub(crate) fn get_attach_entity_packet(
        ...
        self.state
            .get_leash()
            .get_packet(self.state.get_entity_id())
    }

    pub fn get_status_packet(&self, status: i8) -> EntityStatusPacket {
        EntityStatusPacket {
            entity_id: self.state.get_entity_id().get_value(),
            status,
        }
    }
    ...
    pub fn is_on_fire(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::is_on_fire())
    }
    ...
    pub fn is_sneaking(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::is_crouching())
    }
    ...
    pub fn is_sprinting(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::is_sprinting())
    }
    ...
    pub fn is_swimming(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::is_swimming())
    }
    ...
    pub fn is_invisible(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::is_invisible())
    }
    ...
    pub fn is_glowing(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::has_glowing_effect())
    }
    ...
    pub fn set_air_ticks(&mut self, air_ticks: i32) {
        self.state.get_metadata_mut().set(
    ...
    pub fn set_active_hand(&mut self, hand: PlayerHand) {
        self.state.get_metadata_mut().set_flag(
    ...
    pub fn get_pose(&self) -> EntityPose {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::get_pose())
        {
    ...
    pub fn set_pose(&mut self, pose: EntityPose) {
        ...
        self.event_node
            .dispatch("EntityPoseEvent", self.state.get_entity_id());
    }
    ...
    pub fn get_custom_name(&self) -> Option<TextComponent> {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::get_custom_name())
        {
    ...
    pub fn is_custom_name_visible(&self) -> bool {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::custom_name_visible())
        {
    ...
    pub fn is_silent(&self) -> bool {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::is_silent())
        {
    ...
    pub fn has_no_gravity(&self) -> bool {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::has_no_gravity())
        {
    ...
    pub fn get_ticks_frozen(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::ticks_frozen())
        {
    ...
    pub fn is_no_ai(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::mob::no_ai())
    }

    pub fn set_no_ai(&mut self, no_ai: bool) {
        self.state
            .get_metadata_mut()
            .set_flag(&definitions::mob::no_ai(), no_ai);
    }

    pub fn is_left_handed(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::mob::is_left_handed())
    }
    ...
    pub fn is_aggressive(&self) -> bool {
        self.state
            .get_metadata()
            .get_flag(&definitions::mob::is_aggressive())
    }
    ...
    pub fn set_baby(&mut self, baby: bool) {
        ...
        let width = if baby {
            bounding_box.get_width() / 2.0
        } else {
            bounding_box.get_width() * 2.0
        };
        let height = if baby {
            bounding_box.get_height() / 2.0
        } else {
            bounding_box.get_height() * 2.0
        };
    ...
    pub fn get_sniffer_state(&self) -> SnifferState {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_sniffer_drop_seed_at_tick(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_dolphin_treasure_position(&self) -> Position {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_dolphin_has_fish(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_salmon_size(&self) -> SalmonSize {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_goat_has_left_horn(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_pig_boost_time(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_horse_variant(&self) -> HorseVariant {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_camel_last_pose_change_tick(&self) -> i64 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_chested_horse_has_chest(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_llama_strength(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_llama_variant(&self) -> LlamaVariant {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_fox_variant(&self) -> FoxVariant {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_fox_first_uuid(&self) -> Option<Uuid> {
        match self
            .state
            .get_metadata()
    ...
    pub fn set_panda_main_gene(&mut self, gene: PandaGene) {
        self.state.get_metadata_mut().set(
            ...
            MetadataValue::Byte(gene.get_protocol_id() as i8),
        );
    }
    ...
    pub fn get_rabbit_variant(&self) -> RabbitVariant {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_parrot_color(&self) -> ParrotColor {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_turtle_has_egg(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_hoglin_is_immune_to_zombification(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_happy_ghast_is_leash_holder(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_happy_ghast_stays_still(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_piglin_is_immune_to_zombification(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_piglin_is_charging_crossbow(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_creaking_can_move(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_creeper_state(&self) -> CreeperState {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_phantom_size(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_witch_is_drinking_potion(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_wither_center_head_entity_id(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::wither::center_head_target())
        {
    ...
    pub fn get_wither_left_head_entity_id(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::wither::left_head_target())
        {
            MetadataValue::VarInt(entity_id) => entity_id,
            _ => 0,
        }
    ...
    pub fn get_wither_right_head_entity_id(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::wither::right_head_target())
        {
            MetadataValue::VarInt(entity_id) => entity_id,
            _ => 0,
        }
    ...
    pub fn get_wither_invulnerable_time(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_zombie_is_becoming_drowned(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_villager_head_shake_timer(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    fn villager_variant_component(&self) -> Option<VillagerType> {
        match self.state.get_entity_type() {
    ...
    pub fn get_zombie_villager_is_converting(&self) -> bool {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_zombie_villager_data(&self) -> VillagerData {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_copper_golem_weather_state(&self) -> CopperGolemWeatherState {
        match self
            .state
            .get_metadata()
    ...
    pub fn set_copper_golem_weather_state(&mut self, state: CopperGolemWeatherState) {
        self.state.get_metadata_mut().set(
            ...
            MetadataValue::WeatherState(state.get_protocol_id()),
        );
    }
    ...
    pub fn get_shulker_color(&self) -> DyeColor {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_spellcaster_illager_spell(&self) -> SpellcasterIllagerSpell {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_shaking_ticks(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::abstract_vehicle::shaking_power())
        {
    ...
    pub fn get_shaking_direction(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::abstract_vehicle::get_shaking_direction())
        {
    ...
    pub fn set_profile(&mut self, profile: ResolvableProfile) {
        self.state.get_metadata_mut().set(
            &definitions::mannequin::get_profile(),
    ...
    pub fn set_description(&mut self, description: Option<TextComponent>) {
        self.state.get_metadata_mut().set(
            &definitions::mannequin::get_description(),
    ...
    pub fn get_transformation_interpolation_start_delta(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::display::interpolation_delay())
        {
    ...
    pub fn get_transformation_interpolation_duration(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::display::get_transformation_interpolation_duration())
        {
    ...
    pub fn get_position_rotation_interpolation_duration(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::display::position_rotation_interpolation_duration())
        {
    ...
    pub fn get_display_scale(&self) -> Vector3f {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_billboard_render_constraints(&self) -> i8 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_brightness_override(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::display::get_brightness_override())
        {
    ...
    pub fn set_display_view_range(&mut self, view_range: f32) {
        self.state.get_metadata_mut().set(
            &definitions::display::get_view_range(),
    ...
    pub fn set_shadow_radius(&mut self, shadow_radius: f32) {
        self.state.get_metadata_mut().set(
            &definitions::display::get_shadow_radius(),
    ...
    pub fn get_shadow_strength(&self) -> f32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::display::get_shadow_strength())
        {
    ...
    pub fn get_display_width(&self) -> f32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::display::get_width())
        {
    ...
    pub fn get_display_height(&self) -> f32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::display::get_height())
        {
    ...
    pub fn set_display_height(&mut self, height: f32) {
        self.state.get_metadata_mut().set(
            &definitions::display::get_height(),
    ...
    pub fn get_glow_color_override(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::display::get_glow_color_override())
        {
    ...
    pub fn get_display_context(&self) -> i8 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_line_width(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::text_display::get_line_width())
        {
    ...
    pub fn set_line_width(&mut self, line_width: i32) {
        self.state.get_metadata_mut().set(
            &definitions::text_display::get_line_width(),
    ...
    pub fn get_background_color(&self) -> i32 {
        match self
            .state
            .get_metadata()
            .get_value(&definitions::text_display::get_background_color())
        {
    ...
    pub fn set_text_alignment(&mut self, text_alignment: i8) {
        self.state
            .get_metadata_mut()
            .set_byte(&definitions::text_display::get_alignment(), text_alignment);
    }
    ...
    pub fn get_area_effect_cloud_radius(&self) -> f32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_area_effect_cloud_particle(&self) -> Particle {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_custom_minecart_block_state(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_custom_minecart_block_y_position(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_command_block_minecart_command(&self) -> String {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_command_block_minecart_last_output(&self) -> TextComponent {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_end_crystal_beam_target(&self) -> Option<Position> {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_ominous_item_spawner_item(&self) -> Slot {
        match self
            .state
            .get_metadata()
    ...
    pub const fn get_falling_block_state(&self) -> i32 {
        self.state.get_falling_block_state()
    }

    pub fn set_falling_block_state(&mut self, block_state: i32) {
        self.state.set_falling_block_state(block_state);
    }

    pub const fn get_fishing_hook_owner_entity_id(&self) -> Option<EntityId> {
        self.state.get_fishing_hook_owner_entity_id()
    }

    pub fn set_fishing_hook_owner_entity_id(&mut self, owner_entity_id: Option<EntityId>) {
        self.state.set_fishing_hook_owner_entity_id(owner_entity_id);
    }

    pub fn get_slime_size(&self) -> i32 {
        match self
            .state
            .get_metadata()
    ...
    pub fn get_armor_stand_head_rotation(&self) -> Vector3f {
    ...
    pub fn get_armor_stand_body_rotation(&self) -> Vector3f {
    ...
    pub fn get_armor_stand_left_arm_rotation(&self) -> Vector3f {
    ...
    pub fn get_armor_stand_right_arm_rotation(&self) -> Vector3f {
    ...
    pub fn get_armor_stand_right_leg_rotation(&self) -> Vector3f {
    ...
    fn rotation_vector(
        ...
        match self.state.get_metadata().get_value(definition) {
    ...
    pub fn get_eye_height(&self) -> f64 {
        if self.get_pose() == EntityPose::Sleeping {
            return 0.2;
        }
        self.state.get_entity_type().get_eye_height()
    }
    ...
    pub(crate) fn movement_tick(&mut self, world: &WorldSnapshot) -> Option<EntityMovement> {
        ...
        if self.state.get_vehicle().is_some() {
            return None;
        }
        let velocity_per_tick = Velocity(Vector3d {
            x: self.state.get_velocity().0.x / SERVER_TICKS_PER_SECOND,
            y: self.state.get_velocity().0.y / SERVER_TICKS_PER_SECOND,
            z: self.state.get_velocity().0.z / SERVER_TICKS_PER_SECOND,
        });
        ...
        self.state.set_velocity(Velocity(Vector3d {
        ...
        let position = physics.get_new_position();
        let position_changed = position.get_x() != self.state.get_position().get_x()
            || position.get_y() != self.state.get_position().get_y()
            || position.get_z() != self.state.get_position().get_z();
        if !position_changed {
            return None;
        }
        self.set_position(position);
    ...
    fn position_movement_since_synchronization(
        ...
        let scheduled_synchronization_is_imminent = match timing {
            MovementSynchronizationTiming::BeforeEntityTick => self
                .synchronization
                .is_due_by_next_tick(self.state.get_ticks()),
            MovementSynchronizationTiming::AfterEntityTick => {
                self.synchronization.is_due(self.state.get_ticks(), false)
            }
        };
        let should_suppress_movement_packet =
            self.state.get_entity_type().synchronizes_position_only()
                || scheduled_synchronization_is_imminent;
    ...
    pub fn get_relative_start(&self) -> Vector3d {
        Vector3d {
            x: self.state.get_position().get_x() + self.state.get_bounding_box().minimum_x(),
            y: self.state.get_position().get_y() + self.state.get_bounding_box().minimum_y(),
            z: self.state.get_position().get_z() + self.state.get_bounding_box().minimum_z(),
        }
    }

    pub fn get_relative_end(&self) -> Vector3d {
        Vector3d {
            x: self.state.get_position().get_x() + self.state.get_bounding_box().maximum_x(),
            y: self.state.get_position().get_y() + self.state.get_bounding_box().maximum_y(),
            z: self.state.get_position().get_z() + self.state.get_bounding_box().maximum_z(),
        }
    }
    ...
    pub fn get_intersects_box_swept(
        &self,
        ray_start: Vector3d,
    ...
    pub fn schedule_remove_after_ticks(&mut self, delay_ticks: u64) {
        self.delayed_remove_ticks = Some(self.state.get_ticks().saturating_add(delay_ticks));
    }
    ...
    pub const fn is_active(&self) -> bool {
        self.state.get_world().is_some() && !self.state.is_removed()
    }

    pub fn get_scheduler(&mut self) -> &mut ContextScheduler<GenericEntity> {
        &mut self.scheduler
    }
    ...
    pub fn switch_entity_type(&mut self, entity_type: EntityType) {
        self.state.set_entity_type(entity_type);
        *self.state.get_metadata_mut() = MetadataHolder::default();
    ...
    fn refresh_collision_rules(&mut self) {
        let collision_rules = EntityCollisionRules::from_entity_type(self.state.get_entity_type());
    ...
    pub fn spawn_packet(&self) -> SpawnEntityPacket {
        SpawnEntityPacket {
            entity_id: self.state.get_entity_id().get_value(),
            uuid: self.state.get_uuid(),
            entity_type: self.state.get_entity_type().id(),
            x: self.state.get_position().x,
            y: self.state.get_position().y,
            z: self.state.get_position().z,
            velocity: self.spawn_packet_velocity(),
            pitch: EntityAngle(self.state.get_position().pitch),
            yaw: EntityAngle(self.state.get_position().yaw),
            head_yaw: EntityAngle(self.state.get_position().head_yaw),
            data: self.spawn_packet_data(),
        }
    }

    fn spawn_packet_data(&self) -> i32 {
        match self.state.get_entity_type().path() {
            "falling_block" => self.state.get_falling_block_state(),
            "fishing_bobber" => self
                .state
                .get_fishing_hook_owner_entity_id()
    ...
    fn spawn_packet_velocity(&self) -> Velocity {
        match self.state.get_entity_type().path() {
            "llama_spit" | "shulker_bullet" => self.get_protocol_velocity(),
            _ => Velocity(Vector3d {
                x: 0.0,
                y: 0.0,
                z: 0.0,
    ...
    pub fn get_metadata_packet(&self) -> SetEntityDataPacket {
        SetEntityDataPacket::new(
            self.state.get_entity_id().get_value(),
            self.state.get_metadata().get_entries(),
        )
    }
    ...
    pub fn get_velocity_packet(&self) -> EntityVelocityPacket {
        EntityVelocityPacket {
            entity_id: self.state.get_entity_id().get_value(),
            velocity: self.get_protocol_velocity(),
        }
    }

    pub(crate) fn get_protocol_velocity(&self) -> Velocity {
        Velocity(Vector3d {
            x: self.state.get_velocity().0.x / SERVER_TICKS_PER_SECOND,
            y: self.state.get_velocity().0.y / SERVER_TICKS_PER_SECOND,
            z: self.state.get_velocity().0.z / SERVER_TICKS_PER_SECOND,
    ...
    pub fn get_position_delta_packet(
        &self,
        ...
        EntityPositionPacket {
            entity_id: self.state.get_entity_id().get_value(),
            delta_x: EntityPositionPacket::delta(self.state.get_position().x, previous_position.x),
            delta_y: EntityPositionPacket::delta(self.state.get_position().y, previous_position.y),
            delta_z: EntityPositionPacket::delta(self.state.get_position().z, previous_position.z),
            on_ground,
        }
    }

    pub fn get_position_and_rotation_delta_packet(
        &self,
        previous_position: EntityPosition,
        on_ground: bool,
    ...
    pub fn get_rotation_packet(&self, on_ground: bool) -> EntityRotationPacket {
    ...
    pub fn get_head_look_packet(&self) -> EntityHeadLookPacket {
    ...
    pub fn teleport_packet(&self) -> EntityTeleportPacket {
        let protocol_velocity = self.get_protocol_velocity();
        EntityTeleportPacket {
            entity_id: self.state.get_entity_id().get_value(),
            position: self.state.get_position().as_vector(),
            delta: Vector3d {
                x: protocol_velocity.0.x,
                y: protocol_velocity.0.y,
                z: protocol_velocity.0.z,
            },
            yaw: self.state.get_position().yaw,
            pitch: self.state.get_position().pitch,
            flags: 0,
            on_ground: false,
        }
    }

    pub fn get_dirty_metadata_packet(&mut self) -> Option<SetEntityDataPacket> {
        let dirty_entries = self.state.get_metadata_mut().drain_dirty_entries();
    ...
    pub fn remove_packet(&self) -> RemoveEntitiesPacket {
        RemoveEntitiesPacket::new(vec![self.state.get_entity_id().get_value()])
    }

    pub fn update_new_viewer(&self, client: &mut Client) -> io::Result<()> {
        if self.state.get_entity_type() == EntityType::PLAYER {
            PlayerInfoUpdatePacket::add_listed_player(
                self.state.get_uuid(),
                format!("test_player_{}", self.state.get_entity_id().get_value()),
            )
            .dispatch(client)?;
        }
    ...
    fn remove_when_scheduled_tick_has_elapsed(&mut self) {
        let has_reached_scheduled_removal_tick = self
            .delayed_remove_ticks
            .is_some_and(|removal_tick| removal_tick <= self.state.get_ticks());
        if has_reached_scheduled_removal_tick {
            self.remove();
        }
    }
...
impl EntityAerodynamics {
    ...
    pub const fn get_horizontal_air_resistance(self) -> f64 {
        self.horizontal_air_resistance
    }

    pub const fn get_vertical_air_resistance(self) -> f64 {
        self.vertical_air_resistance
    }

    pub const fn get_gravity(self) -> f64 {
        self.gravity
    }
}
...
impl EntityPosition {
    ...
    pub const fn get_head_yaw(self) -> f32 {
        self.head_yaw
    }
    ...
    pub const fn get_offset(self, x: f64, y: f64, z: f64) -> Self {
        Self {
            x: self.x + x,
            y: self.y + y,
            z: self.z + z,
...
fn swept_end(ray_start: Vector3d, ray_end: Vector3d, moving_box: EntityBoundingBox) -> Vector3d {
    Vector3d {
        x: ray_start.x.max(ray_end.x) + moving_box.get_width(),
        y: ray_start.y.max(ray_end.y) + moving_box.get_height(),
