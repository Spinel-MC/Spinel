use crate::common::parsers::get_crate_namespace;
use crate::common::utils::{get_base_path, resolve_id};
...
pub fn declare_module_dependency_logic(input: TokenStream) -> TokenStream {
    ...
    for (i, dep_lit) in parser.dependencies.iter().enumerate() {
        ...
        let sanitized_subject = qualified_subject
            .replace(":", "_")
            .replace("-", "_")
            .to_uppercase();
        ...
        expanded.extend(quote! {
            ...
            static #static_name: #event_path::RegisteredModuleDependency =
                #event_path::RegisteredModuleDependency {
                    subject_module: #qualified_subject,
                    dependent_on: #qualified_dependency,
                };
            #event_path::inventory::submit!(&#static_name);
        });
    }
